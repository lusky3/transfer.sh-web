...
describe('GenericPreview', () => {
  ...
  it('renders archive icon for zip files', () => {
    const { container } = render(<GenericPreview filename="archive.zip" contentType="application/zip" />)
    expect(container.querySelector('svg')).toBeInTheDocument()
  })

  it('renders archive icon for tar files', () => {
    const { container } = render(<GenericPreview filename="archive.tar.gz" contentType="application/x-tar" />)
    expect(container.querySelector('svg')).toBeInTheDocument()
  })

  it('renders spreadsheet icon for xlsx files', () => {
    const { container } = render(<GenericPreview filename="data.xlsx" contentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" />)
    expect(container.querySelector('svg')).toBeInTheDocument()
  })
